    ...
    <>
      <section className="section" id="education">
        <div className="container">
          <SectionHeader 
            title="Education" 
            subtitle="My academic foundation that supports my engineering career."
            alignment="center"
          ...
          <div className="editorial-card fade-up" style={{ padding: '2.5rem', maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div className="editorial-card fade-up" style={{ padding: '2.5rem', maxWidth: '800px', margin: '0 auto' }}>
            {certifications.map((cert, idx) => (
              <div key={idx} style={{ marginBottom: idx !== certifications.length - 1 ? '2rem' : 0, borderBottom: idx !== certifications.length - 1 ? '1px solid var(--border-color)' : 'none', paddingBottom: idx !== certifications.length - 1 ? '2rem' : 0 }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', lineHeight: '1.6' }}>{cert.description}</p>
              </div>
            ))}
          </div>
        </div>
